...
class OmniGenPipeline:
    ...
    def from_pretrained(cls, model_name, vae_path: str=None, Quantization: bool = False):
        ...
        if Quantization:
            ...
            logger.info("Applied 8-bit quantization to the model")
    ...
    def __call__(
        ...
        if seed is not None:
            ...
            generator = None
        latents = torch.randn(num_prompt, 4, latent_size_h, latent_size_w, device=device, generator=generator)
        latents = torch.cat([latents]*(1+num_cfg), 0).to(dtype)

        if input_images is not None and self.model_cpu_offload: self.vae.to(device)
        input_img_latents = []
        if separate_cfg_infer:
            for temp_pixel_values in input_data['input_pixel_values']:
                temp_input_latents = []
                for img in temp_pixel_values:
                    img = self.vae_encode(img.to(device), dtype)
        ...
        if self.model_cpu_offload:
            ...
            for buffer_name, buffer in self.model.named_buffers():
                setattr(self.model, buffer_name, buffer.to(device))
        # else:
        #     self.model.to(device)
